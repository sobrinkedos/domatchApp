...
export default function ChampionsModal({ isOpen, onClose, champions }) {
  ...
  // Verificação mais robusta com valores padrão
  const defaultChampions = {
    topPlayers: [],
    topTeam: { players: [], wins: 0, points: 0 }
  };

  const { topPlayers = [], topTeam = { players: [], wins: 0, points: 0 } } = champions || defaultChampions;
                ...
                {topPlayers && topPlayers.map((player, index) => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{player?.name || 'Jogador'}</p>
                        <p className="text-sm text-gray-500">
                          {player?.wins || 0} vitórias · {player?.points || 0} pontos
                        </p>
                      </div>
                    </div>
                    {index === 0 && (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-gray-900">
                      {topTeam?.players?.join(' & ') || 'Sem dupla campeã'}
                    </p>
                    <p className="text-sm text-gray-500">
                      {topTeam?.wins || 0} vitórias · {topTeam?.points || 0} pontos
                    </p>
                  </div>
                  <TrophyIcon className="h-8 w-8 text-yellow-400" />
                </div>
